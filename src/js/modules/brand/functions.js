...
$(document).ready(function () {
  ...
  $('form').on('submit', function (e) {
    ...
    emailjs
      .sendForm(service, templateId, e.target, user)
      .then(
        res => {
          if (res.status === 200) {
        ...
        err => {
          if (err) {
            submitBtn.attr('disabled', false)
            form.append(`<small class="form-error">Ошибка при отправке заявки! Попробуйте позже</small>`)
  ...
  $('.section-title').each((index, title) => {
    ...
    previewText?.filter(word => (word !== '' && word !== '<br>') && word).forEach((word, i) => {
      $(title).append(`<span><span data-aos="fade-up-mask" data-aos-delay="${6100 + i * 50}">${word}</span></span>`)
    })
  })
